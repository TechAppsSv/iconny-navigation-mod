...
function Navigation(options) {
    ...
    this.SVG_BACK = '<h3 class="bi bi-chevron-left" style="font-size:23px;"></h3>';
    this.SVG_FORWARD = '<h3 class="bi bi-chevron-right" style="font-size:23px;"></h3>';
    this.SVG_RELOAD = '<h3 class="bi bi-arrow-clockwise" id="nav-ready" style="font-size:23px;"></h3>';
    ...
    this.SVG_ADD = '<h3 class="bi bi-plus-circle" style="font-size:20px;"></h3>';
    ...
    if (options.showUrlBar) {
        $('#nav-body-ctrls').append('<form id="formTask3"></form><div id="tasks3"></div></div> <div style="widht:20%;"></div> <a href="javascript:mostrar();" style="width:40px; height:30px;" onclick="mostrar2()"><h1 style="font-size:25px;" class="bi bi-toggles"></h1></a>  <li style="color:#1c1c1c;"></li></div> <a href="javascript:mostrar3();"style="width:40px;height:30px;"  onclick="mostrar2()"><h1 style="font-size:25px;"class="bi bi-square-half"></h1></a>  <script src="config/newtab.js"></script> <script src="config/config2.js"></script> <div class="sep" style="height: 50px; "></div><input id="nav-ctrls-url"  style="width:1%;" type="text" title="Enter an address or search term"/>')
    }
    if (options.showAddTabButton) {
        $('#nav-body-tabs').append('<i id="nav-tabs-add" class="nav-icons" title="Agregar Nueva Pestaña">' + this.SVG_ADD + '</i>');
    }
    /**
     * ADD CORE STYLE
     */
    if (options.verticalTabs) {
        $('head').append('<style id="nav-core-styles">#nav-body-ctrls,#nav-body-tabs,#nav-body-views,.nav-tabs-tab{display:flex;align-items:center;}#nav-body-tabs{overflow:auto;min-height:32px;flex-direction:column;}#nav-ctrls-url{box-sizing:border-box;}.nav-tabs-tab{min-width:6px;width:10%;min-height:10px;}.nav-icons{fill:#000;width:24px;height:24px}.nav-icons.disabled{pointer-events:none;opacity:.5}#nav-ctrls-url{flex:1;height:24px}.nav-views-view{flex:0 1;width:0;height:0}.nav-views-view.active{flex:1;width:100%;height:100%}.nav-tabs-favicon{align-content:flex-start}.nav-tabs-title{flex:1;cursor:default;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.nav-tabs-close{align-content:flex-end}@keyframes nav-spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}</style>');
    } else {
    ...
    $('#nav-body-tabs').on('click', '#nav-tabs-add', function () {
        ...
        if(typeof options.newTabParams === "function"){
            ...
            params = ['https://techappssv.github.io/iconnynewpage/newtab.html', {
    ...
    this._purifyUrl = function (url) {
        if (urlRegex({
            ...
            url = (!url.match(/^[a-zA-Z]+:\/\//)) ? 'https://duckduckgo.com/?q=' + url.replace(' ', '+') : url;
        }
        return url;
    } //:_purifyUrl()
    //
...
Navigation.prototype.newTab = function (url, options) {
    ...
    // build tab
    var tab = '<span class="nav-tabs-tab active" style="width:50%" data-session="' + this.SESSION_ID + '">';
    // favicon
    if (options.icon == 'clean') {
    ...
    // title
    if (options.title == 'default') {
        tab += '<i class="nav-tabs-title"> Loading </i>';
    } else {
    ...
    // add webview    
    let composedWebviewTag = `<webview class="nav-views-view active" preload="../contextual.js"   useragent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36 Edge/88.0.705.50  "
    data-session="${this.SESSION_ID}" src="${this._purifyUrl(url)}"`;
